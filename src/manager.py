...
class Manager(BaseAssistant):
    def __init__(self, resources_path, prompt):
        ...
        self.manager_assistant = self.client.beta.assistant.create(
            name="Manager",
    ...
    def extract_topic(self, prompt):
        completion = self.client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant that helps people extract the main topic from a given prompt. In the response, please provide . For instance if the prompt is: 'Generate a dataset for QnA based on football', The main topic is: Football.",
                },
                {
                    "role": "user",
                    "content": prompt,
        ...
        topic = completion.choices[
            0
        ].message  # check if the format of returned value is correct
        return topic

    def generate_dataset(self):
        questions, file_ids = self.questioner.generate_questions(self.topic)
        answers = self.answerer.generate_answers(
            questions, file_ids
        )  # not sure if topics should go as an argument
